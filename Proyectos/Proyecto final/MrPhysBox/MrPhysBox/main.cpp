...
int main(int argc, char* args[])
{
	...
	simulation->init("Halo Engine", SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED, 640, 360, false);
	...
	*/
	...
	/*
		CREAMOS EL CUERPO DEL PERSONAJE
	*/
	...
	*/
	...
	floorBox.SetAsBox(640.0f * 0.5f / ppm, 37.0f * 0.5f / ppm);
	...
	*/
	b2BodyDef plat1Def;
	plat1Def.position.Set(88.0f / ppm, 248.0f / ppm);

	b2Body* plat1Body = gameWorld.CreateBody(&plat1Def);

	b2PolygonShape plat1Box;
	plat1Box.SetAsBox(176.0f * 0.5f / ppm, 16.0f * 0.5f / ppm);
	...
	*/
	b2BodyDef plat2Def;
	plat2Def.position.Set(296.0f / ppm, 184.0f / ppm);

	b2Body* plat2Body = gameWorld.CreateBody(&plat2Def);

	b2PolygonShape plat2Box;
	plat2Box.SetAsBox(112.0f * 0.5f / ppm, 16.0f * 0.5f / ppm);
	...
	*/
	b2BodyDef plat3Def;
	plat3Def.position.Set(528.0f / ppm, 88.0f / ppm);

	b2Body* plat3Body = gameWorld.CreateBody(&plat3Def);

	b2PolygonShape plat3Box;
	plat3Box.SetAsBox(224.0f * 0.5f / ppm, 16.0f * 0.5f / ppm);
